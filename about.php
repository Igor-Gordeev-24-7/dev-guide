...
$request_limit = 60; // Ограничение на 1 запрос в минуту
if (!isset($_SESSION['last_request_time'])) {
    $_SESSION['last_request_time'] = 0; // Инициализация времени последнего запроса
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Проверка частоты запросов
    if (time() - $_SESSION['last_request_time'] < $request_limit) {
        $remaining_time = $request_limit - (time() - $_SESSION['last_request_time']);
        $feedback = "<p>Слишком много запросов. Пожалуйста, подождите еще $remaining_time секунд.</p>";
    } else {
        ...
        if (!empty($honeypot)) {
            $feedback = "<p>Ошибка: некорректный запрос.</p>";
        } else {
...
<body>
            ...
            <div class="about__wrapper wrapper">
                ...
                <div class="about__container">
                    ...
                    <form action="about.php" method="post" class="about__form" id="contact-form">
                        ...
                        <input class="about__input-submit" type="submit" value="Отправить" id="submit-button">
                    </form>
                </div>
            </div>
        </section>
    ...
    // Проверка, нужно ли запускать таймер
    <?php if (time() - $_SESSION['last_request_time'] < $request_limit): ?>
    let remainingTime = <?= $request_limit - (time() - $_SESSION['last_request_time']); ?>;
    const submitButton = document.getElementById('submit-button');
    submitButton.disabled = true;
    submitButton.value = `Отправить через ${remainingTime} сек`;
